...
use test_utils::MessagePredicate;
...
use {
    ...
    futures_timer::Delay,
    never::Never,
    std::{error::Error, pin::Pin, sync::atomic::AtomicBool, time::Duration},
};
...
async fn it_concats_1_async_finite_listenable_source() {
    ...
    let source = callbag_concat::concat!(source_a);
...
async fn it_concats_2_async_finite_listenable_sources() {
    ...
    let source = callbag_concat::concat!(source_a, source_b);
    source(Message::Handshake(sink));
...
async fn it_returns_a_source_that_disposes_upon_upwards_end() {
    ...
    let source = callbag_concat::concat!(make_source());
    let sink = make_sink();
    source(Message::Handshake(sink));
...
async fn it_propagates_source_error_to_sink_and_doesnt_subscribe_to_next_source() {
    ...
    let source = callbag_concat::concat!(make_source(), make_source());
    let sink = make_sink();
    source(Message::Handshake(sink));
